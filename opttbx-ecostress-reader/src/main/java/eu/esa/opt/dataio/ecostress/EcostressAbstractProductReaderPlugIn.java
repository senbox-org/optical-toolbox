 ...
 * @author adraghici
 */
public abstract class EcostressAbstractProductReaderPlugIn implements ProductReaderPlugIn {
    ...
    private static final Logger logger = Logger.getLogger(EcostressAbstractProductReaderPlugIn.class.getName());
    ...
    private static boolean checkAndInitHdf5Lib() {
        final ClassLoader classLoader = EcostressAbstractProductReaderPlugIn.class.getClassLoader();

        final String classResourceName = "/" + EcostressAbstractProductReaderPlugIn._H5_CLASS_NAME.replace('.', '/') + ".class";
        if (EcostressAbstractProductReaderPlugIn.class.getResource(classResourceName) != null) {
            try {
                Class.forName(EcostressAbstractProductReaderPlugIn._H5_CLASS_NAME, true, classLoader);
                return true;
            } catch (Throwable error) {
                logger.warning(MessageFormat.format("{0}: HDF-5 library not available: {1}: {2}", EcostressAbstractProductReaderPlugIn.class, error.getClass(), error.getMessage()));
     ...
     */
    protected abstract boolean isInputContentValid(EcostressFile ecostressFile);

    /**
     * Creates the ECOSTRESS reader instance
     * @return the ECOSTRESS reader instance
     */
    public abstract EcostressAbstractProductReader createReaderInstance();
    ...
    /**
     * Gets the format name from reader plugin subclass
    ...
    public String[] getFormatNames() {
        return new String[]{FORMAT_NAME_ECOSTRESS, getFormatName()};
    }

    /**
     * Checks whether the reader input object is a valid ECOSTRESS product
     * @param input the reader input object
     * @return {@code true} when the the reader input object is a valid ECOSTRESS product
     */
    protected boolean isInputValid(Object input) {
        final EcostressFile ecostressFile = getEcostressFile(input);
        final boolean fileNameValid = isInputEcostressFileNameValid(ecostressFile.getName());
        if (fileNameValid) {
            return isInputContentValid(ecostressFile);
        }
        return false;
    }
