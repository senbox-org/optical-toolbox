        ...
        try (NetcdfFile ncfile = NetcdfFileOpener.open(inputFile)) {
            if (ncfile != null) {
                ...
                if (instrumentName != null) {
                    if (instrumentName.toString().contains("Oceansat OCM2 Level-1B Data")) {
                        if (DEBUG) {
                        ...
                        if (DEBUG) {
                            System.out.println("# Unrecognized instrument name=[" + instrumentName + "]: " + inputFile);
                        }
                    }
                } else {
                    if (DEBUG) {
                        System.out.println("# Missing Instrument_Name attribute': " + inputFile);
                    }
                }
            } else {
                if (DEBUG) {
