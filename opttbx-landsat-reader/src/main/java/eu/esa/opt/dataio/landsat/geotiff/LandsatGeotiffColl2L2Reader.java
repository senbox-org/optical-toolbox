...
public class LandsatGeotiffColl2L2Reader extends AbstractProductReader {
    ...
    private ProductSensorSpecification productSpecs;
...
//    }
    ...
    protected Product readProductNodesImpl() throws IOException {
        ...
        final String sensorName = getSensorName(mtlFile);
        productSpecs = ProductSensorSpecification.getProductSpecification(sensorName); 
    ...
    private static String getSensorName(File mtlfile) {
        String filename = mtlfile.getName();
        if (filename.length() < 4) {
        	return "08";
        }
    	return filename.substring(2, 4);
    }
...
//            }
            ...
            final String bandDescription = productSpecs.getBandDescription(bandname);
            if (bandDescription != null) {
                band.setDescription(bandDescription);
            }
            final Double scalingFactor = productSpecs.getScalingFactor(bandname);
            if (scalingFactor != null) {
                band.setScalingFactor(scalingFactor);
            }
            final Double addOffsets = productSpecs.getAddOffset(bandname);
            if (addOffsets != null) {
                band.setScalingOffset(addOffsets);
            }
            final Float wavelength = productSpecs.getWavelength(bandname);
            if (wavelength != null) {
                band.setSpectralWavelength(wavelength);
            }
            final Float bandwidth = productSpecs.getBandwidth(bandname);
            if (bandwidth != null) {
                band.setSpectralBandwidth(bandwidth);
            }
            final Double fillValue = productSpecs.getFillValue(bandname);
            if (fillValue != null) {
                band.setNoDataValue(fillValue);
                band.setNoDataValueUsed(true);
            }
            final String unit = productSpecs.getUnit(bandname);
            if (unit != null) {
                band.setUnit(unit);
            }
            final Map<String, FlagCodingArgs> flagCodingData = productSpecs.getFlagCoding(bandname);
            if (flagCodingData != null) {
                final FlagCoding flagCoding = createFlagCoding(bandname, flagCodingData.values());
    ...
    private Color getColorForMask(String bandName, String flagName) {
        return productSpecs.getFlagCoding(bandName).get(flagName).getColor();
    }

    private float getTransparencyForMask(String bandName, String flagName) {
        return productSpecs.getFlagCoding(bandName).get(flagName).getTransparency();
    }
