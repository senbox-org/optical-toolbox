...
@OperatorMetadata(
        ...
        description = "Normalized Difference Impervious Surface Index Green",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class NdisigOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "ndisig";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Ndisig", rectangle.height);
        try {
            ...
            Tile ndisig = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile ndisigFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    ndisigValue = (t-(g+n+s1)/3.0f)/(t+(g+n+s1)/3.0f);
                    ndisig.setSample(x, y, computeFlag(x, y, ndisigValue, ndisigFlags));
                }
