...
@OperatorMetadata(
        ...
        description = "Normalized Burn Ratio Thermal 2",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class Nbrt2Op extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "nbrt2";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Nbrt2", rectangle.height);
        try {
            ...
            Tile nbrt2 = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile nbrt2Flags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    nbrt2Value = ((n/(t/10000.0f))-s2)/((n/(t/10000.0f))+s2);
                    nbrt2.setSample(x, y, computeFlag(x, y, nbrt2Value, nbrt2Flags));
                }
