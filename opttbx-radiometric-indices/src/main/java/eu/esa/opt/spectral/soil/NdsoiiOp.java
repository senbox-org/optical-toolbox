...
@OperatorMetadata(
        ...
        description = "Normalized Difference Soil Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class NdsoiiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "ndsoii";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Ndsoii", rectangle.height);
        try {
            ...
            Tile ndsoii = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile ndsoiiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    ndsoiiValue = (s2-g)/(s2+g);
                    ndsoii.setSample(x, y, computeFlag(x, y, ndsoiiValue, ndsoiiFlags));
                }
