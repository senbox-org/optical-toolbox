...
public class RdviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "rdvi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Rdvi", rectangle.height);
        try {
            ...
            Tile rdvi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile rdviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    rdviValue = (n-r)/pow((n+r),0.5f);
                    rdvi.setSample(x, y, computeFlag(x, y, rdviValue, rdviFlags));
                }
