...
public class GariOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "gari";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Gari", rectangle.height);
        try {
            ...
            Tile gari = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile gariFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    gariValue = (n-(g-(b-r)))/(n-(g+(b-r)));
                    gari.setSample(x, y, computeFlag(x, y, gariValue, gariFlags));
                }
