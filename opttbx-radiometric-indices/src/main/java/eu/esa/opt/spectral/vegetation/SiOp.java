...
@OperatorMetadata(
        ...
        description = "Shadow Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class SiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "si";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Si", rectangle.height);
        try {
            ...
            Tile si = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile siFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    siValue = pow(((1.0f-b)*(1.0f-g)*(1.0f-r)),(1.0f/3.0f));
                    si.setSample(x, y, computeFlag(x, y, siValue, siFlags));
                }
