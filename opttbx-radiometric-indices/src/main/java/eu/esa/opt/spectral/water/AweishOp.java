...
@OperatorMetadata(
        ...
        description = "Automated Water Extraction Index with Shadows Elimination",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class AweishOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "aweish";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Aweish", rectangle.height);
        try {
            ...
            Tile aweish = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile aweishFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    aweishValue = b+2.5f*g-1.5f*(n+s1)-0.25f*s2;
                    aweish.setSample(x, y, computeFlag(x, y, aweishValue, aweishFlags));
                }
