...
@OperatorMetadata(
        ...
        description = "Modified Triangular Vegetation Index 2",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class Mtvi2Op extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "mtvi2";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Mtvi2", rectangle.height);
        try {
            ...
            Tile mtvi2 = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile mtvi2Flags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    mtvi2Value = (1.5f*(1.2f*(n-g)-2.5f*(r-g)))/pow((pow((2.0f*n+1),2)-(6.0f*n-5*pow(r,0.5f))-0.5f),0.5f);
                    mtvi2.setSample(x, y, computeFlag(x, y, mtvi2Value, mtvi2Flags));
                }
