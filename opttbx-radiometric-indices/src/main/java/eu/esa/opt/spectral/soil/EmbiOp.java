...
@OperatorMetadata(
        ...
        description = "Enhanced Modified Bare Soil Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class EmbiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "embi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Embi", rectangle.height);
        try {
            ...
            Tile embi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile embiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    embiValue = ((((s1-s2-n)/(s1+s2+n))+0.5f)-((g-s1)/(g+s1))-0.5f)/((((s1-s2-n)/(s1+s2+n))+0.5f)+((g-s1)/(g+s1))+1.5f);
                    embi.setSample(x, y, computeFlag(x, y, embiValue, embiFlags));
                }
