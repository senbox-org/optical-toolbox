...
@OperatorMetadata(
        ...
        description = "Red Chromatic Coordinate",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class RccOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "rcc";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Rcc", rectangle.height);
        try {
            ...
            Tile rcc = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile rccFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    rccValue = r/(r+g+b);
                    rcc.setSample(x, y, computeFlag(x, y, rccValue, rccFlags));
                }
