...
@OperatorMetadata(
        ...
        description = "Normalized Green",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class NormgOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "normg";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Normg", rectangle.height);
        try {
            ...
            Tile normg = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile normgFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    normgValue = g/(n+g+r);
                    normg.setSample(x, y, computeFlag(x, y, normgValue, normgFlags));
                }
