...
public class GvmiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "gvmi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Gvmi", rectangle.height);
        try {
            ...
            Tile gvmi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile gvmiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    gvmiValue = ((n+0.1f)-(s2+0.02f))/((n+0.1f)+(s2+0.02f));
                    gvmi.setSample(x, y, computeFlag(x, y, gvmiValue, gvmiFlags));
                }
