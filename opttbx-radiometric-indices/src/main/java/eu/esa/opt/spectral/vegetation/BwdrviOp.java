...
@OperatorMetadata(
        ...
        description = "Blue Wide Dynamic Range Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class BwdrviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "bwdrvi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Bwdrvi", rectangle.height);
        try {
            ...
            Tile bwdrvi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile bwdrviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    bwdrviValue = (alpha*n-b)/(alpha*n+b);
                    bwdrvi.setSample(x, y, computeFlag(x, y, bwdrviValue, bwdrviFlags));
                }
