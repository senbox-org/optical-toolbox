...
@OperatorMetadata(
        ...
        description = "Modified Non-Linear Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class MnliOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "mnli";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Mnli", rectangle.height);
        try {
            ...
            Tile mnli = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile mnliFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    mnliValue = (1+l)*(pow(n,2)-r)/(pow(n,2)+r+l);
                    mnli.setSample(x, y, computeFlag(x, y, mnliValue, mnliFlags));
                }
