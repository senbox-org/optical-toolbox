...
public class WriOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "wri";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Wri", rectangle.height);
        try {
            ...
            Tile wri = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile wriFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    wriValue = (g+r)/(n+s1);
                    wri.setSample(x, y, computeFlag(x, y, wriValue, wriFlags));
                }
