...
/**
 * Operator class for Tcariosavi705
 *
 * @author Adrian Draghici
 */
@OperatorMetadata(
        alias = "Tcariosavi705Op",
        version = "1.0",
        category = "Optical/Thematic Land Processing/Vegetation Spectral Indices",
        description = "TCARI/OSAVI Ratio (705 and 750 nm)",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class Tcariosavi705Op extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "tcariosavi705";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Tcariosavi705", rectangle.height);
        try {
            ...
            Tile tcariosavi705 = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile tcariosavi705Flags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));

            float tcariosavi705Value;
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    tcariosavi705Value = (3*((re2-re1)-0.2f*(re2-g)*(re2/re1)))/(1.16f*(re2-re1)/(re2+re1+0.16f));
                    tcariosavi705.setSample(x, y, computeFlag(x, y, tcariosavi705Value, tcariosavi705Flags));
                }
