...
public class UiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "ui";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Ui", rectangle.height);
        try {
            ...
            Tile ui = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile uiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    uiValue = (s2-n)/(s2+n);
                    ui.setSample(x, y, computeFlag(x, y, uiValue, uiFlags));
                }
