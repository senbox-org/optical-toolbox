...
@OperatorMetadata(
        ...
        description = "Normalized Shortwave Infrared Difference Soil-Moisture",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class NsdsOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "nsds";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Nsds", rectangle.height);
        try {
            ...
            Tile nsds = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile nsdsFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    nsdsValue = (s1-s2)/(s1+s2);
                    nsds.setSample(x, y, computeFlag(x, y, nsdsValue, nsdsFlags));
                }
