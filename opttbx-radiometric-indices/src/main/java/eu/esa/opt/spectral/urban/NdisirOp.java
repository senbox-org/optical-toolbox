...
@OperatorMetadata(
        ...
        description = "Normalized Difference Impervious Surface Index Red",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class NdisirOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "ndisir";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Ndisir", rectangle.height);
        try {
            ...
            Tile ndisir = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile ndisirFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    ndisirValue = (t-(r+n+s1)/3.0f)/(t+(r+n+s1)/3.0f);
                    ndisir.setSample(x, y, computeFlag(x, y, ndisirValue, ndisirFlags));
                }
