...
@OperatorMetadata(
        ...
        description = "Normalized Difference Impervious Surface Index with NDWI",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class NdisindwiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "ndisindwi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Ndisindwi", rectangle.height);
        try {
            ...
            Tile ndisindwi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile ndisindwiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));

            float ndisindwiValue;
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    ndisindwiValue = (t-(((g-n)/(g+n))+n+s1)/3.0f)/(t+(((g-n)/(g+n))+n+s1)/3.0f);
                    ndisindwi.setSample(x, y, computeFlag(x, y, ndisindwiValue, ndisindwiFlags));
                }
