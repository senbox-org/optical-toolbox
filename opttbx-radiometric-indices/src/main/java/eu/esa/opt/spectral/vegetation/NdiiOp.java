...
public class NdiiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "ndii";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Ndii", rectangle.height);
        try {
            ...
            Tile ndii = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile ndiiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    ndiiValue = (n-s1)/(n+s1);
                    ndii.setSample(x, y, computeFlag(x, y, ndiiValue, ndiiFlags));
                }
