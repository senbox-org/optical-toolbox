...
public class CsitOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "csit";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Csit", rectangle.height);
        try {
            ...
            Tile csit = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile csitFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    csitValue = n/(s2*t/10000.0f);
                    csit.setSample(x, y, computeFlag(x, y, csitValue, csitFlags));
                }
