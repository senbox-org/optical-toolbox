...
@OperatorMetadata(
        ...
        description = "Green Ratio Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class GrviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "grvi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Grvi", rectangle.height);
        try {
            ...
            Tile grvi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile grviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    grviValue = n/g;
                    grvi.setSample(x, y, computeFlag(x, y, grviValue, grviFlags));
                }
