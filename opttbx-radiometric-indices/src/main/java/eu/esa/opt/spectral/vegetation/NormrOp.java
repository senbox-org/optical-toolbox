...
@OperatorMetadata(
        ...
        description = "Normalized Red",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class NormrOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "normr";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Normr", rectangle.height);
        try {
            ...
            Tile normr = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile normrFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    normrValue = r/(n+g+r);
                    normr.setSample(x, y, computeFlag(x, y, normrValue, normrFlags));
                }
