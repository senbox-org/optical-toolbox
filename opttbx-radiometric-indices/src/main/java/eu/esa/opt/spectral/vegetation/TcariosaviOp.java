...
@OperatorMetadata(
        ...
        description = "TCARI/OSAVI Ratio",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class TcariosaviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "tcariosavi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Tcariosavi", rectangle.height);
        try {
            ...
            Tile tcariosavi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile tcariosaviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));

            float tcariosaviValue;
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    tcariosaviValue = (3*((re1-r)-0.2f*(re1-g)*(re1/r)))/(1.16f*(n-r)/(n+r+0.16f));
                    tcariosavi.setSample(x, y, computeFlag(x, y, tcariosaviValue, tcariosaviFlags));
                }
