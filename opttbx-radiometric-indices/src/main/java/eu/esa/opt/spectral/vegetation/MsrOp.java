...
public class MsrOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "msr";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Msr", rectangle.height);
        try {
            ...
            Tile msr = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile msrFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    msrValue = (n/r-1)/pow((n/r+1),0.5f);
                    msr.setSample(x, y, computeFlag(x, y, msrValue, msrFlags));
                }
