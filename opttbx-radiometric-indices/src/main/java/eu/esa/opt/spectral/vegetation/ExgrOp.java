...
@OperatorMetadata(
        ...
        description = "ExG - ExR Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class ExgrOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "exgr";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Exgr", rectangle.height);
        try {
            ...
            Tile exgr = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile exgrFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    exgrValue = (2.0f*g-r-b)-(1.3f*r-g);
                    exgr.setSample(x, y, computeFlag(x, y, exgrValue, exgrFlags));
                }
