...
@OperatorMetadata(
        ...
        description = "Visible Green-Based Built-Up Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class VgnirbiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "vgnirbi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Vgnirbi", rectangle.height);
        try {
            ...
            Tile vgnirbi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile vgnirbiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    vgnirbiValue = (g-n)/(g+n);
                    vgnirbi.setSample(x, y, computeFlag(x, y, vgnirbiValue, vgnirbiFlags));
                }
