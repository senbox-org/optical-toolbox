...
@OperatorMetadata(
        ...
        description = "Transformed Difference Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class TdviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "tdvi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Tdvi", rectangle.height);
        try {
            ...
            Tile tdvi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile tdviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    tdviValue = 1.5f*((n-r)/pow((pow(n,2.0f)+r+0.5f),0.5f));
                    tdvi.setSample(x, y, computeFlag(x, y, tdviValue, tdviFlags));
                }
