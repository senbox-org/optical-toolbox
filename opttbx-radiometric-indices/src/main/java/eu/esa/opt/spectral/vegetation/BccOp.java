...
@OperatorMetadata(
        ...
        description = "Blue Chromatic Coordinate",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class BccOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "bcc";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Bcc", rectangle.height);
        try {
            ...
            Tile bcc = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile bccFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    bccValue = b/(r+g+b);
                    bcc.setSample(x, y, computeFlag(x, y, bccValue, bccFlags));
                }
