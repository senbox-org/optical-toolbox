...
public class LswiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "lswi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Lswi", rectangle.height);
        try {
            ...
            Tile lswi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile lswiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    lswiValue = (n-s1)/(n+s1);
                    lswi.setSample(x, y, computeFlag(x, y, lswiValue, lswiFlags));
                }
