...
public class MsiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "msi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Msi", rectangle.height);
        try {
            ...
            Tile msi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile msiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    msiValue = s1/n;
                    msi.setSample(x, y, computeFlag(x, y, msiValue, msiFlags));
                }
