...
@OperatorMetadata(
        ...
        description = "Green Soil Adjusted Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class GsaviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "gsavi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Gsavi", rectangle.height);
        try {
            ...
            Tile gsavi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile gsaviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    gsaviValue = (1.0f+l)*(n-g)/(n+g+l);
                    gsavi.setSample(x, y, computeFlag(x, y, gsaviValue, gsaviFlags));
                }
