...
@OperatorMetadata(
        ...
        description = "Normalized Rapeseed Flowering Index Green",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class NrfigOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "nrfig";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Nrfig", rectangle.height);
        try {
            ...
            Tile nrfig = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile nrfigFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    nrfigValue = (g-s2)/(g+s2);
                    nrfig.setSample(x, y, computeFlag(x, y, nrfigValue, nrfigFlags));
                }
