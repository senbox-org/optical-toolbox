...
public class GliOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "gli";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Gli", rectangle.height);
        try {
            ...
            Tile gli = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile gliFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    gliValue = (2.0f*g-r-b)/(2.0f*g+r+b);
                    gli.setSample(x, y, computeFlag(x, y, gliValue, gliFlags));
                }
