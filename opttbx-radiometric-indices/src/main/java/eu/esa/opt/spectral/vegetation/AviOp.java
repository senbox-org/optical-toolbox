...
public class AviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "avi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Avi", rectangle.height);
        try {
            ...
            Tile avi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile aviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    aviValue = pow(n*(1.0f-r)*(n-r),1.0f/3.0f);
                    avi.setSample(x, y, computeFlag(x, y, aviValue, aviFlags));
                }
