...
@OperatorMetadata(
        ...
        description = "Disease-Water Stress Index 5",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class Dswi5Op extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "dswi5";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Dswi5", rectangle.height);
        try {
            ...
            Tile dswi5 = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile dswi5Flags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    dswi5Value = (n+g)/(s1+r);
                    dswi5.setSample(x, y, computeFlag(x, y, dswi5Value, dswi5Flags));
                }
