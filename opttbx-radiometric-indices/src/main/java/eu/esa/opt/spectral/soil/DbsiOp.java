...
@OperatorMetadata(
        ...
        description = "Dry Bareness Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class DbsiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "dbsi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Dbsi", rectangle.height);
        try {
            ...
            Tile dbsi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile dbsiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    dbsiValue = ((s1-g)/(s1+g))-((n-r)/(n+r));
                    dbsi.setSample(x, y, computeFlag(x, y, dbsiValue, dbsiFlags));
                }
