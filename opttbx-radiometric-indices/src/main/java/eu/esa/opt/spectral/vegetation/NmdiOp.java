...
@OperatorMetadata(
        ...
        description = "Normalized Multi-band Drought Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class NmdiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "nmdi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Nmdi", rectangle.height);
        try {
            ...
            Tile nmdi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile nmdiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    nmdiValue = (n-(s1-s2))/(n+(s1-s2));
                    nmdi.setSample(x, y, computeFlag(x, y, nmdiValue, nmdiFlags));
                }
