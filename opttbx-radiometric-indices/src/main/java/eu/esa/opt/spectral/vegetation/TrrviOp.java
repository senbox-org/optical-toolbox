...
@OperatorMetadata(
        ...
        description = "Transformed Red Range Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class TrrviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "trrvi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Trrvi", rectangle.height);
        try {
            ...
            Tile trrvi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile trrviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    trrviValue = ((re2-r)/(re2+r))/(((n-r)/(n+r))+1.0f);
                    trrvi.setSample(x, y, computeFlag(x, y, trrviValue, trrviFlags));
                }
