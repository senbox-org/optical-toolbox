...
public class GrndviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "grndvi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Grndvi", rectangle.height);
        try {
            ...
            Tile grndvi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile grndviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    grndviValue = (n-(g+r))/(n+(g+r));
                    grndvi.setSample(x, y, computeFlag(x, y, grndviValue, grndviFlags));
                }
