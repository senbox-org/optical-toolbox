...
@OperatorMetadata(
        ...
        description = "Transformed Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class TviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "tvi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Tvi", rectangle.height);
        try {
            ...
            Tile tvi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile tviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    tviValue = pow((((n-r)/(n+r))+0.5f),0.5f);
                    tvi.setSample(x, y, computeFlag(x, y, tviValue, tviFlags));
                }
