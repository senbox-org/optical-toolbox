...
public class NddiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "nddi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Nddi", rectangle.height);
        try {
            ...
            Tile nddi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile nddiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    nddiValue = (((n-r)/(n+r))-((g-n)/(g+n)))/(((n-r)/(n+r))+((g-n)/(g+n)));
                    nddi.setSample(x, y, computeFlag(x, y, nddiValue, nddiFlags));
                }
