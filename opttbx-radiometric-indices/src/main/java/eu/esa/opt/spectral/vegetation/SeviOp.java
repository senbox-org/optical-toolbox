...
@OperatorMetadata(
        ...
        description = "Shadow-Eliminated Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class SeviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "sevi";

	@Parameter(label = "L Parameter", defaultValue = "1.0F", description = "The l parameter.")
	private float l;

	@Parameter(label = "Fdelta Parameter", defaultValue = "1.0F", description = "The fdelta parameter.")
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Sevi", rectangle.height);
        try {
            ...
            Tile sevi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile seviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    seviValue = (n/r)+fdelta*(1.0f/r);
                    sevi.setSample(x, y, computeFlag(x, y, seviValue, seviFlags));
                }
