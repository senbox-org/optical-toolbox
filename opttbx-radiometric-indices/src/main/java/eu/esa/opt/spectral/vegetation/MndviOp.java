...
public class MndviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "mndvi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Mndvi", rectangle.height);
        try {
            ...
            Tile mndvi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile mndviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    mndviValue = (n-s2)/(n+s2);
                    mndvi.setSample(x, y, computeFlag(x, y, mndviValue, mndviFlags));
                }
