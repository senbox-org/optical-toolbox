...
@OperatorMetadata(
        ...
        description = "Red Green Blue Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class RgbviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "rgbvi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Rgbvi", rectangle.height);
        try {
            ...
            Tile rgbvi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile rgbviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    rgbviValue = (pow(g,2.0f)-b*r)/(pow(g,2.0f)+b*r);
                    rgbvi.setSample(x, y, computeFlag(x, y, rgbviValue, rgbviFlags));
                }
