...
@OperatorMetadata(
        ...
        description = "Normalized NIR",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class NormnirOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "normnir";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Normnir", rectangle.height);
        try {
            ...
            Tile normnir = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile normnirFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    normnirValue = n/(n+g+r);
                    normnir.setSample(x, y, computeFlag(x, y, normnirValue, normnirFlags));
                }
