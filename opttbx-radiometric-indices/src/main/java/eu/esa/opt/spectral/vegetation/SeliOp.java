...
@OperatorMetadata(
        ...
        description = "Sentinel-2 LAI Green Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class SeliOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "seli";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Seli", rectangle.height);
        try {
            ...
            Tile seli = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile seliFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    seliValue = (pow(n,2)-re1)/(pow(n,2)+re1);
                    seli.setSample(x, y, computeFlag(x, y, seliValue, seliFlags));
                }
