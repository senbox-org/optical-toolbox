...
@OperatorMetadata(
        ...
        description = "Green Chromatic Coordinate",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class GccOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "gcc";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Gcc", rectangle.height);
        try {
            ...
            Tile gcc = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile gccFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    gccValue = g/(r+g+b);
                    gcc.setSample(x, y, computeFlag(x, y, gccValue, gccFlags));
                }
