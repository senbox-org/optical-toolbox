...
@OperatorMetadata(
        ...
        description = "Visible Red-Based Built-Up Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class VrnirbiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "vrnirbi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Vrnirbi", rectangle.height);
        try {
            ...
            Tile vrnirbi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile vrnirbiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    vrnirbiValue = (r-n)/(r+n);
                    vrnirbi.setSample(x, y, computeFlag(x, y, vrnirbiValue, vrnirbiFlags));
                }
