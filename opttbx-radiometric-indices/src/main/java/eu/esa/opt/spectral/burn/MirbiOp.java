...
@OperatorMetadata(
        ...
        description = "Mid-Infrared Burn Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class MirbiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "mirbi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Mirbi", rectangle.height);
        try {
            ...
            Tile mirbi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile mirbiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    mirbiValue = 10.0f*s2-9.8f*s1+2.0f;
                    mirbi.setSample(x, y, computeFlag(x, y, mirbiValue, mirbiFlags));
                }
