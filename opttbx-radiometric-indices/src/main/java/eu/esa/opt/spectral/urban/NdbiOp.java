...
public class NdbiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "ndbi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Ndbi", rectangle.height);
        try {
            ...
            Tile ndbi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile ndbiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    ndbiValue = (s1-n)/(s1+n);
                    ndbi.setSample(x, y, computeFlag(x, y, ndbiValue, ndbiFlags));
                }
