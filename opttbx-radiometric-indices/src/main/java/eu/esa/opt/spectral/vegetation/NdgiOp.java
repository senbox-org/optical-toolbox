...
public class NdgiOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "ndgi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Ndgi", rectangle.height);
        try {
            ...
            Tile ndgi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile ndgiFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    ndgiValue = (((lambdaN-lambdaR)/(lambdaN-lambdaG))*g+(1.0f-((lambdaN-lambdaR)/(lambdaN-lambdaG)))*n-r)/(((lambdaN-lambdaR)/(lambdaN-lambdaG))*g+(1.0f-((lambdaN-lambdaR)/(lambdaN-lambdaG)))*n+r);
                    ndgi.setSample(x, y, computeFlag(x, y, ndgiValue, ndgiFlags));
                }
