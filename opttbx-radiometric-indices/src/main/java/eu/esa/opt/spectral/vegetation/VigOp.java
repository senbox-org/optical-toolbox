...
@OperatorMetadata(
        ...
        description = "Vegetation Index Green",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class VigOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "vig";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Vig", rectangle.height);
        try {
            ...
            Tile vig = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile vigFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    vigValue = (g-r)/(g+r);
                    vig.setSample(x, y, computeFlag(x, y, vigValue, vigFlags));
                }
