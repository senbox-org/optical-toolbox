...
@OperatorMetadata(
        ...
        description = "Specific Leaf Area Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class SlaviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "slavi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Slavi", rectangle.height);
        try {
            ...
            Tile slavi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile slaviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    slaviValue = n/(r+s2);
                    slavi.setSample(x, y, computeFlag(x, y, slaviValue, slaviFlags));
                }
