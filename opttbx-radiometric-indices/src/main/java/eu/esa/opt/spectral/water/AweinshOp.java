...
public class AweinshOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "aweinsh";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Aweinsh", rectangle.height);
        try {
            ...
            Tile aweinsh = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile aweinshFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    aweinshValue = 4.0f*(g-s1)-0.25f*n+2.75f*s2;
                    aweinsh.setSample(x, y, computeFlag(x, y, aweinshValue, aweinshFlags));
                }
