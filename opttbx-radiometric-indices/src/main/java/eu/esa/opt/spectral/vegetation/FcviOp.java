...
@OperatorMetadata(
        ...
        description = "Fluorescence Correction Vegetation Index",
        authors = "Adrian Draghici",
        copyright = "Copyright (C) 2025 by CS Group ROMANIA")
public class FcviOp extends BaseIndexOp {

    // constants
    public static final String BAND_NAME = "fcvi";
    ...
    public void computeTileStack(Map<Band, Tile> targetTiles, Rectangle rectangle, ProgressMonitor pm) throws OperatorException {
        pm.beginTask("Computing Fcvi", rectangle.height);
        try {
            ...
            Tile fcvi = targetTiles.get(getBandWithSuffix(targetProduct, "_" + BAND_NAME));
            Tile fcviFlags = targetTiles.get(targetProduct.getBand(FLAGS_BAND_NAME));
            ...
            for (int y = rectangle.y; y < rectangle.y + rectangle.height; y++) {
                for (int x = rectangle.x; x < rectangle.x + rectangle.width; x++) {
                    ...
                    fcviValue = n-((r+g+b)/3.0f);
                    fcvi.setSample(x, y, computeFlag(x, y, fcviValue, fcviFlags));
                }
