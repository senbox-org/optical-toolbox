...
//        double sin_sun = sin(toRadians(sun_zeni));
        ...
        if (validPixel) {
            ...
            if (outputRtosaGcAann || outputOos) {
                ...
                rtosa_aann = ArrayMath.a_exp(log_rtosa_aann);
            }
            //double[] rtosa_aaNNrat = adiv(rtosa_aann, r_tosa);
            //rtosa_aaNNrat_a(ipix,:)=rtosa_aaNNrat;
            rtosa_oos = 0;
            if (outputOos) {
                double[] abs_diff_log_rtosa = ArrayMath.a_abs(log_rtosa, log_rtosa_aann);
                rtosa_oos = ArrayMath.a_max(abs_diff_log_rtosa);
            }
        ...
        */
            ...
            if (outputRpath || deriveRwFromPathAndTransmittance) {
                ...
                rpath_nn = ArrayMath.a_exp(log_rpath_nn);
            }
