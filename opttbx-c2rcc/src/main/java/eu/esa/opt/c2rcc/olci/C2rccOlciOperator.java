...
import eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.Result;
...
import static eu.esa.opt.c2rcc.C2rccCommons.addBand;
import static eu.esa.opt.c2rcc.C2rccCommons.addVirtualBand;
import static eu.esa.opt.c2rcc.C2rccCommons.fetchOzone;
import static eu.esa.opt.c2rcc.C2rccCommons.fetchSurfacePressure;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.DEFAULT_OLCI_WAVELENGTH;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_ADET_AT_MAX;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_ADET_AT_MIN;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_AGELB_AT_MAX;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_AGELB_AT_MIN;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_APIG_AT_MAX;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_APIG_AT_MIN;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_BPART_AT_MAX;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_BPART_AT_MIN;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_BWIT_AT_MAX;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_BWIT_AT_MIN;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_CLOUD;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_IOP_OOR;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_KD489_AT_MAX;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_KD489_OOR;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_KDMIN_AT_MAX;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_KDMIN_OOR;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_RHOW_OOR;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_RHOW_OOS;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_RTOSA_OOR;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_RTOSA_OOS;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.FLAG_INDEX_VALID_PE;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.IDX_iop_rw;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.IDX_iop_unciop;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.IDX_iop_uncsumiop_unckd;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.IDX_rtosa_aann;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.IDX_rtosa_rpath;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.IDX_rtosa_rw;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.IDX_rtosa_trans;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.IDX_rw_iop;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.IDX_rw_kd;
import static eu.esa.opt.c2rcc.olci.C2rccOlciAlgorithm.IDX_rw_rwnorm;
...
public class C2rccOlciOperator extends PixelOperator implements C2rccConfigurable {
    ...
    */
    ...
    public void initialiseElevationModel() {
        if (this.useSnapDem) {
            ElevationModelDescriptor elevationModelDesc = ElevationModelRegistry.getInstance().getDescriptor(this.demName);

            if (elevationModelDesc != null) {
                elevationModel = elevationModelDesc.createDem(Resampling.BILINEAR_INTERPOLATION);
            } else {
                ElevationModelDescriptor elevationModelDescDefault = ElevationModelRegistry.getInstance().getDescriptor(DEM_NAME_GETASSE30);
                elevationModel = elevationModelDescDefault.createDem(Resampling.BILINEAR_INTERPOLATION);
            }
        }
    }
