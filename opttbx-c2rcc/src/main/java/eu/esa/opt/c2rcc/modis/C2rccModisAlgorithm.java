...
import eu.esa.opt.c2rcc.util.ArrayMath;
...
//    static final int FLAG_INDEX_KDMIN_AT_MAX = 19;
    ...
    public Result processPixel(double[] toa_ref,
        ...
        double[] log_rtosa = ArrayMath.a_log(r_tosa);
        ...
        double[] rw = ArrayMath.a_exp(log_rw);

         // (9.5) test out of scope spectra with autoassociative neural network
        double[] log_rtosa_aann = rtoa_aaNN7.get().calc(nn_in);
        double[] rtosa_aann = ArrayMath.a_exp(log_rtosa_aann);
        double[] rtosa_aaNNrat = ArrayMath.a_div(rtosa_aann, r_tosa);
        //rtosa_aaNNrat_a(ipix,:)=rtosa_aaNNrat;

        int flags = 0;

        // (9.6.1) set rho_toa out of scope flag
        double rtosa_aaNNrat_min = ArrayMath.a_min(rtosa_aaNNrat);
        double rtosa_aaNNrat_max = ArrayMath.a_max(rtosa_aaNNrat);
        ...
        double[] iops_nn1 = ArrayMath.a_exp(log_iops_nn1);
