...
import eu.esa.opt.c2rcc.C2rccCommons;
import eu.esa.opt.c2rcc.C2rccConfigurable;
import eu.esa.opt.c2rcc.ancillary.AtmosphericAuxdata;
import eu.esa.opt.c2rcc.ancillary.AtmosphericAuxdataBuilder;
import eu.esa.opt.c2rcc.util.NNUtils;
import eu.esa.opt.c2rcc.util.RgbProfiles;
...
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.DEFAULT_WAVELENGTH;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_ADET_AT_MAX;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_ADET_AT_MIN;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_AGELB_AT_MAX;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_AGELB_AT_MIN;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_APIG_AT_MAX;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_APIG_AT_MIN;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_BPART_AT_MAX;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_BPART_AT_MIN;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_BWIT_AT_MAX;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_BWIT_AT_MIN;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_CLOUD;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_IOP_OOR;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_KD489_AT_MAX;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_KD489_OOR;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_KDMIN_AT_MAX;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_KDMIN_OOR;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_RHOW_OOR;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_RHOW_OOS;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_RTOSA_OOR;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_RTOSA_OOS;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.FLAG_INDEX_VALID_PE;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.IDX_iop_rw;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.IDX_iop_unciop;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.IDX_iop_uncsumiop_unckd;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.IDX_rtosa_aann;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.IDX_rtosa_rpath;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.IDX_rtosa_rw;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.IDX_rtosa_trans;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.IDX_rw_iop;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.IDX_rw_kd;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.IDX_rw_rwnorm;
import static eu.esa.opt.c2rcc.landsat.C2rccLandsat8Algorithm.Result;
...
public class C2rccLandsat8Operator extends PixelOperator implements C2rccConfigurable {
    ...
    */
    ...
    private static final String DEFAULT_DEM_NAME = "Copernicus 90m Global DEM";
    private static final String DEM_NAME_COPERNICUS30 = "Copernicus 30m Global DEM";
    private static final String DEM_NAME_GETASSE30 = "GETASSE30";
    ...
    protected void configureTargetProduct(ProductConfigurer productConfigurer) {
        ...
        if (outputRtoa) {
            for (int i = 0; i < L8_BAND_COUNT; i++) {
                final Band band = C2rccCommons.addBand(targetProduct, "rtoa_" + (i + 1), "1", "Top-of-atmosphere reflectance");
        ...
        if (outputRtosaGc) {
            for (int i = 0; i < L8_BAND_COUNT; i++) {
                Band band = C2rccCommons.addBand(targetProduct, "rtosa_gc_" + (i + 1), "1", "Gas corrected top-of-atmosphere reflectance, input to AC");
                ensureSpectralProperties(band, i);
        ...
        if (outputRtosaGcAann) {
            for (int i = 0; i < L8_BAND_COUNT; i++) {
                Band band = C2rccCommons.addBand(targetProduct, "rtosagc_aann_" + (i + 1), "1", "Gas corrected top-of-atmosphere reflectance, output from AANN");
                ensureSpectralProperties(band, i);
                band.setValidPixelExpression(validPixelExpression);
            }
        ...
        if (outputRpath) {
            for (int i = 0; i < L8_BAND_COUNT; i++) {
                Band band = C2rccCommons.addBand(targetProduct, "rpath_" + (i + 1), "1", "Path-radiance reflectances");
                ensureSpectralProperties(band, i);
                band.setValidPixelExpression(validPixelExpression);
            }
        ...
        if (outputTdown) {
            for (int i = 0; i < L8_BAND_COUNT; i++) {
                Band band = C2rccCommons.addBand(targetProduct, "tdown_" + (i + 1), "1", "Transmittance of downweling irradiance");
                ensureSpectralProperties(band, i);
                band.setValidPixelExpression(validPixelExpression);
            }
        ...
        if (outputTup) {
            for (int i = 0; i < L8_BAND_COUNT; i++) {
                Band band = C2rccCommons.addBand(targetProduct, "tup_" + (i + 1), "1", "Transmittance of upweling irradiance");
                ensureSpectralProperties(band, i);
                band.setValidPixelExpression(validPixelExpression);
            }
        ...
        if (outputAcReflectance) {
            for (int i = 0; i < L8_BAND_COUNT; i++) {
                final Band band;
                if (outputAsRrs) {
                    band = C2rccCommons.addBand(targetProduct, "rrs_" + (i + 1), "sr^-1",
                                   "Atmospherically corrected Angular dependent remote sensing reflectances");
                } else {
                    band = C2rccCommons.addBand(targetProduct, "rhow_" + (i + 1), "1", "Atmospherically corrected Angular dependent water leaving reflectances");
                }
                ensureSpectralProperties(band, i);
                band.setValidPixelExpression(validPixelExpression);
            }
        ...
        if (outputRhown) {
            for (int i = 0; i < L8_BAND_COUNT; i++) {
                final Band band = C2rccCommons.addBand(targetProduct, "rhown_" + (i + 1), "1", "Normalized water leaving reflectances");
                ensureSpectralProperties(band, i);
                band.setValidPixelExpression(validPixelExpression);
            }
        ...
        if (outputOos) {
            final Band oos_rtosa = C2rccCommons.addBand(targetProduct, "oos_rtosa", "1",
        ...
        Band iop_atot = C2rccCommons.addVirtualBand(targetProduct, "iop_atot", "iop_apig + iop_adet + iop_agelb", "m^-1",
        ...
        if (outputKd) {
            ...
            kd_z90max = C2rccCommons.addVirtualBand(targetProduct, "kd_z90max", "1 / kdmin", "m",
        ...
        Band conc_tsm = C2rccCommons.addVirtualBand(targetProduct, "conc_tsm", "iop_bpart * " + TSMfakBpart + " + iop_bwit * " + TSMfakBwit, "g m^-3",
                                       "Total suspended matter dry weight concentration");
        Band conc_chl = C2rccCommons.addVirtualBand(targetProduct, "conc_chl", "pow(iop_apig, " + CHLexp + ") * " + CHLfak, "mg m^-3",
        ...
        if (outputUncertainties) {
            ...
            Band unc_tsm = C2rccCommons.addVirtualBand(targetProduct, "unc_tsm", "unc_btot * " + TSMfakBpart, "g m^-3",
                                          "Uncertainty of total suspended matter (TSM) dry weight concentration");
            Band unc_chl = C2rccCommons.addVirtualBand(targetProduct, "unc_chl", "pow(unc_apig, " + CHLexp + ") * " + CHLfak, "mg m^-3",
            ...
            if (outputKd) {
                ...
                Band unc_kd_z90max = C2rccCommons.addVirtualBand(targetProduct, "unc_kd_z90max", "abs(kd_z90max - 1.0 / abs(kdmin - unc_kdmin))", "m",
