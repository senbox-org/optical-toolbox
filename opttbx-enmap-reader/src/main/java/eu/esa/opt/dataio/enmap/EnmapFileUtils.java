...
//            Pattern.compile(L1C_BASEFILENAME + "-SPECTRAL_IMAGE.(BSQ|BIP|BIL)") // only in case of HDR
    ...
    public static InputStream getInputStream(VirtualDir dataDir, String fileName, boolean isCompliantProduct) throws IOException {
        return dataDir.getInputStream(getRelativePath(dataDir, fileName, isCompliantProduct));
    }

    public static String getRelativePath(VirtualDir dataDir, String fileName, boolean isCompliantProduct) throws IOException {
        String relPath = fileName;
        if (isCompliantProduct) {
            return relPath;
        }
    ...
    public static boolean checkIfProductIsNonCompliant(VirtualDir dataDir) throws IOException {
        String productName = FileUtils.getFilenameWithoutExtension(dataDir.getBaseFile().getName());

        for (String fileName : dataDir.listAllFiles()) {
